...
| Перший Core-on-Core | Корисна зміна власного harness або source; відхилена шкідлива мутація; persistence й recovery | AF-RSI-010–019 з транзитивними залежностями | Recursive method improvement або користь для будь-якої задачі |
...
| Lokiravia CLD007–019 | Brief → мала версія → Godot → checks → Play → feedback → v2/restore | W0 споживає прийняті starter/adapter/checkpoint/Play контракти; не створює другий build pipeline |
...
Аудит зафіксував current consumer pin Core `480849f78957bb6b2fd7ab341300d54955aeabb8`. Нові contract/adapters не вважаються доступними через те, що Core HEAD новіший. Upstream build qualification, exact compatibility receipts, consumer upgrade і fallback на попередню версію — окремий перехід, описаний у [спільному контракті](platform-world-contract.md).

## 5. Покращення creator-продукту Lokiravia

Світовий feedback та проблема автора — різні сигнали. Приклад: автор хотів мирну гру, а звіт системи красивий, проте кнопка Play не відкриває потрібну версію. Покращення квестів тут не вирішить проблеми. Lokiravia зберігає початковий brief, відмови автора й exact build; пропонує зміну власного шляху brief/preview/feedback/restore. Reusable experiment machinery приходить з Core, а критерії авторського успіху належать Lokiravia.

Перший creator experiment спирається на чинні CLD007/008/015/017/018/020 і Core experiment/evidence contracts. Гіпотеза: пояснення scope із двома конкретними playable варіантами допомагає автору самостійно отримати задуману малу гру. Baseline — чинний шлях, challenger — точна версія пропозиції. До запуску визначити первинну вісь: незалежне завершення шляху **або** менші зусилля за не гіршої fidelity. Hard floors: автор бачить, що виключено; правильний Play version; restore не губить ідею; жодного додаткового платного запуску без доступного budget.

Пілотний план: 8–12 авторів із власними короткими brief, зіставлені завдання й порядок показу, окремо записані допомога фасилітатора, час, відмови й несподівані витрати. Це якісний пошук проблем, не доказ ринкового effect size. Відкриті питання та спостереження передують числовому scale-test. Якщо автор закінчив швидше лише тому, що продукт спростив його задум без згоди, кандидат відхиляється. Якщо змінилася аудиторія або саме завдання продукту, потрібна нова версія product hypothesis; не слід перейменовувати старий провал на успіх.
...
До реалізації потрібні рішення про підтримуваний локальний runtime профіль, доступні resources, набір перших non-game задач і спосіб залучення авторів/гравців. Вони не блокують документаційну розробку. Жоден огляд літератури або сучасний benchmark не замінює майбутній експеримент на цих продуктах.
